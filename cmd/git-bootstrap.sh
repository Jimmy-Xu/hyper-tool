...
function setup_centos(){
  ...
  echo
  echo "Setup under CentOS/Redhat..."
  echo
...
function setup_ubuntu(){
  ...
  echo
  echo "Setup under Debian/Ubuntu..."
  echo
  ...
  sudo apt-get -y install git git-core git-man git-arch git-extras git-flow git-gui git-review git-sh
...
function main(){
  ...
  then
   ##########Linux##########
    ...
    ##check distributor and osversion
    DISTRIBUTOR=''
    OSVERSION=''
    ...
    then
      ...
      then
        if [ -f /etc/os-release ]
        then
          RELEASE_CFG=/etc/os-release
        fi
        ALLOS=`cat \`ls /etc/*release | grep -v os-release\`  | head -n 1`
        DISTRIBUTOR=`cat \`ls /etc/*release | grep -v os-release\` | head -n 1 | cut -d '=' -f 2| cut -d ' ' -f 1`
        OSVERSION=`cat \`ls /etc/*release | grep -v os-release\` | head -n 1 | cut -d '=' -f 2| cut -d ' ' -f 3  | awk 'BEGIN{FS="[:. \t]"}{print $1}' `
      else
    ...
    echo "Your OS : ${DISTRIBUTOR} | ${OSVERSION} | ${HOSTTYPE} "

    if [ "${DISTRIBUTOR}" == "CentOS" -o "${DISTRIBUTOR}" == "Red" ]
    then
      echo "${DISTRIBUTOR}"
    elif [ "${DISTRIBUTOR}" == "Amazon" ]
    then
      echo "Amazon Linux"
    elif [ "${DISTRIBUTOR}" == "Ubuntu" -o "${DISTRIBUTOR}" == "Debian"  ]
    then
      echo "${DISTRIBUTOR}"
    else
      quit "Only support CentOS, Debian, Ubuntu or Amazon,Quit!"
    fi
  ...
  then
    ...
    echo "Install auto-completion for Linux(${DISTRIBUTOR})..."
    if [ `cat ~/.bashrc | grep 'source ${GIT_COMPLETION}' | wc -l`  -eq 0 ]
    then
      echo -e "\nsource ${GIT_COMPLETION}" >> ~/.bashrc
      echo "" >> ~/.bashrc
      echo ">>Add 'source ${GIT_COMPLETION}' to ~/.bashrc OK!"
    else
      echo ">>Already add 'source ${GIT_COMPLETION}' to ~/.bashrc,nothing to do!"
    fi
  elif [ "${UNAME}" == "Darwin" ]
  then

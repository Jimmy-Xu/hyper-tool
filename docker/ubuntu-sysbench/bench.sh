...
function generate_test_case() {
  ...
  if [ ${EXEC_MODE} == "live"  ];then
    ...
    SYS_MEM_CASE=( "${MAX_REQUESTS} 95 1 global 100G 1M"  "${MAX_REQUESTS} 95 ${CPU_NUM} global 200G 1M" )
...
function memtest_cmd() {
  echo "${SYSBENCH} --test=memory --num-threads=$3 --memory-block-size=1M --memory-total-size=100G run"
...
function do_io_test() {
  ...
  do
    ...
    for test_mode in seqwr seqrewr seqrd rndrd rndwr rndrw
    do
